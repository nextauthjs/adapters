...
function printSchema() {
  return new Promise(async (resolve) => {
    ...
    connection.query(
      TABLES.map(
        (table) =>
          `SELECT column_name, data_type, character_maximum_length, is_nullable, column_default FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = '${table}' ORDER BY ordinal_position`
      ).join(";"),
      (error, result) => {
        if (error) {
          throw error
        ...
        result[0].rows.forEach((column) => {
          users[column.column_name] = getColumnSchema(column)
        })
        result[1].rows.forEach((column) => {
          accounts[column.column_name] = getColumnSchema(column)
        })
        result[2].rows.forEach((column) => {
          sessions[column.column_name] = getColumnSchema(column)
        })
        result[3].rows.forEach((column) => {
          verification_requests[column.column_name] = getColumnSchema(column)
        })
...
;(async () => {
  ...
  if (compareResult === true) {
    ...
    console.log(
      ...
      JSON.stringify(testResultSchema, null, 2)
    )
    process.exit(1)
  }
})()
