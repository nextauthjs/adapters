    ...
    connection.query(
      ...
      (error, result) => {
        ...
        result[0].forEach((column) => {
          users[column.Field] = getColumnSchema(column)
        })
        result[1].forEach((column) => {
          accounts[column.Field] = getColumnSchema(column)
        })
        result[2].forEach((column) => {
          sessions[column.Field] = getColumnSchema(column)
        })
        result[3].forEach((column) => {
          verification_requests[column.Field] = getColumnSchema(column)
        })
